...
// ---------- Badge tiers with ranges ----------
const BADGE_TIERS = [
  { min: 500, max: null, label: "Legendary Guardian", emoji: "🏆", border: "border-yellow-400", text: "text-yellow-900", bg: "bg-gradient-to-br from-yellow-50 to-amber-100", next: null },
  { min: 300, max: 499, label: "Guardian", emoji: "🌟", border: "border-purple-400", text: "text-purple-900", bg: "bg-gradient-to-br from-purple-50 to-indigo-100", next: 500 },
  { min: 100, max: 299, label: "Archivist", emoji: "💎", border: "border-cyan-400", text: "text-cyan-900", bg: "bg-gradient-to-br from-cyan-50 to-blue-100", next: 300 },
  { min: 50, max: 99, label: "Trusted Contributor", emoji: "🥈", border: "border-slate-400", text: "text-slate-800", bg: "bg-gradient-to-br from-slate-50 to-slate-100", next: 100 },
  { min: 10, max: 49, label: "Contributor", emoji: "✅", border: "border-emerald-400", text: "text-emerald-900", bg: "bg-gradient-to-br from-emerald-50 to-green-100", next: 50 },
  { min: 1, max: 9, label: "Witness", emoji: "🌱", border: "border-stone-300", text: "text-stone-700", bg: "bg-gradient-to-br from-stone-50 to-stone-100", next: 10 },
];
...
function rangeLabel(tier: typeof BADGE_TIERS[number]) {
    ...
    <div className="min-h-screen bg-background grain-overlay">
      ...
      {showRegPrompt && (
        <ContributorRegistrationPrompt
          ...
          onDismiss={() => setShowRegPrompt(false)}
        />
      )}
      ...
      <section className="border-b border-border bg-card">
        <div className="container mx-auto px-6 py-12">
          ...
          {/* Badge tiers legend with ranges */}
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-8">
            {BADGE_TIERS.map((tier) => (
              ...
              >
                ...
                <span className="text-[9px] font-mono font-semibold text-muted-foreground">{rangeLabel(tier)} records</span>
              </div>
            ))}
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {/* Anyone can register as a contributor */}
            {!isRegistered() && (
              <button
                onClick={() => setShowRegPrompt(true)}
                className="inline-block border border-primary text-primary px-6 py-3 text-xs font-semibold tracking-widest uppercase hover:bg-primary hover:text-primary-foreground transition-colors duration-200"
              >
        ...
        <section className="border-b border-border bg-card/50">
          <div className="container mx-auto px-6 py-10 max-w-2xl">
            <div className="flex items-start gap-6">
              ...
              <div>
                <h2 className="text-2xl mb-2" style={{ fontFamily: "'Fraunces', serif" }}>Thank you.</h2>
      ...
      <section className="container mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {MOCK_CONTRIBUTORS.map((c) => {
            ...
            return (
              <div key={c.id} className={`${badge.bg} border ${badge.border} p-5 flex gap-4 items-start`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className={`text-[9px] font-bold uppercase tracking-wider ${badge.text} opacity-70 mb-1`}>
                    {badge.label} · {rangeLabel(badge)} records
                  </div>
                  {c.public && c.city !== "—" && (
                  ...
                  {badge.next && (
                    <div className="mt-2">
                      <div className="h-1 bg-black/10 overflow-hidden">
                        <div className="h-full bg-current opacity-40 transition-all" style={{ width: `${Math.min((c.count / badge.next) * 100, 100)}%` }} />
                      </div>
